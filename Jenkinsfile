pipeline {
  ...
  environment {
    ...
    CONTAINER_NAME = 'consort-forum-service'
    DOCKER_NETWORK = 'consort-infra_consort-network'
    LOGS_DIR       = '/app/logs/forum-service'
      ...
      steps {
        sh '''
          ...
          docker run -d \
            ...
            -p ${APP_PORT}:${APP_PORT} \
            -v ${LOGS_DIR}:/app/logs/forum-service \
            --env-file ${ENV_FILE} \
            -e TZ=Asia/Tashkent \
            ...
            -e SPRING_DATA_REDIS_HOST=consort-redis-forum-service \
